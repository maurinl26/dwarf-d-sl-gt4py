...
MODULE MOD_FFT
  ...
  USE MOD_PARAMETER, ONLY : GXPT,GYPT,NSMAX,MSMAX,RPI,RDX,RDY,RLX,RLY,&
                          & ZERO,ONE,TWO,NSMAX_ORO,MSMAX_ORO
...
CONTAINS
  ...
  SUBROUTINE FFTINV_UV(U,V)
    ...
    INTEGER                                      :: IX,IY
    REAL(kind=GFT_Prec)                          :: SC
    COMPLEX(kind=GFT_Prec), DIMENSION(0:GXPT,1)  :: HATU,XU,HATV,V
  ...
  !*****************************************************************************!
  !*****************************************************************************!
  SUBROUTINE ELLIPTIC_TRUNC_ORO(P)
    ...
    P(NSMAX_ORO+1:GXPT,:) = ZERO
    P(:,MSMAX_ORO+1:GYPT) = ZERO

    DO NX=1,NSMAX_ORO
       MY_TRUNC = INT(MSMAX_ORO*DSQRT(ONE-((REAL(NX,8)/REAL(NSMAX_ORO,8))**2)))
       P(NX,MY_TRUNC+1:MSMAX_ORO) = ZERO
    END DO    
    
  END SUBROUTINE ELLIPTIC_TRUNC_ORO
  !****************************************************************************!
  !****************************************************************************!
  !****************************************************************************!
  !****************************************************************************!
  SUBROUTINE FFTORO_2D(P,DPDX,DPDY,LALIAZ)

    IMPLICIT NONE

    REAL(8), DIMENSION(GXPT,GYPT), INTENT(INOUT) :: P
    REAL(8), DIMENSION(GXPT,GYPT), INTENT(INOUT) :: DPDX,DPDY
    LOGICAL, OPTIONAL,             INTENT(IN)    :: LALIAZ
