...
def boundaries(
    ...
    # Fixed boundaries
        ...
        point = (
            point * (1 - right_exceed) * (1 - left_exceed)
            + xmin * left_exceed
            + xmax * right_exceed
        )

        indice = (
            indice * (1 - right_exceed) * (1 - left_exceed)
            + 0 * left_exceed
            + (nx - 1) * right_exceed
        )
...
def sl_xy(
    ...
    lx_d, ly_d, i_d, j_d, vx_e, vy_e = lagrangian_search(
        x=x, y=y, I=I, J=J, vx_e=vx_e, vy_e=vy_e, vx=vx, vy=vy
    )
...
if __name__ == "__main__":
    ...
    for jstep in range(1, NITMP):
        ...
        # Estimations
        vx_e, vy_e, tracer_e = sl_xy(I=I, J=J, x=x, y=y, vx=vx, vy=vy, vx_e=vx_e, vy_e=vy_e, tracer=tracer)
