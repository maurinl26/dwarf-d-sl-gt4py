...
def run_sl_2D(lsettls: bool, nsiter: int, nitmp: int):
    ...
    print(f"Shape vx : {vx.shape}, Shape vy : {vy.shape}")
    ...
    print(f"Shape, tracer : {tracer.shape}")
    ...
    print(f"Max of tracer field,  X : {config.xcr[imax, jmax]:.2f} Y : {config.ycr[imax, jmax]:.2f}")
