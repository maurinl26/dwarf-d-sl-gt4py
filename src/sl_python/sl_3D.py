...
from sl_python.sl_2D import dep_search_1dS
...
def departure_search(
    ...
    lz, k_d = dep_search_1d(K, vz_e, vz_tmp, dz, dth)
...
def sl_init(
    ...
    return vx, vy, vz, vx_e, vy_e, vz_e
...
def sl_xy(
    ...
    # Recherche semi lag
    lx_d, ly_d, lz_d, i_d, j_d, k_d = lagrangian_search(
        config=config,
        vx_e=vx_e,
        vy_e=vy_e,
        vx=vx,
    ...
    logging.info(f"Tracer (t + dt) : mean {tracer_e.mean()}")
