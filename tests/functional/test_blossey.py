...
def sl_driver(
    ...
    while t < config.model_endtime:
        ...
        logging.info(f"Time : {100*t/(config.model_endtime - config.model_starttime):.02f}%")
        ...
        if (T / 4) <= t < (T / 4) + config.dt:
            plot_blossey(config.xcr, config.ycr, vx, vy, tracer, Path(figure_dir, f"blossey_{t:.03f}.pdf"))

        if (T / 2) <= t < (T / 2) + config.dt:
            plot_blossey(config.xcr, config.ycr, vx, vy, tracer, Path(figure_dir, f"blossey_{t:.03f}.pdf"))
            
        if (0.75 * T) <= t < (0.75 * T) + config.dt:
            plot_blossey(config.xcr, config.ycr, vx, vy, tracer, Path(figure_dir, f"blossey_{t:.03f}.pdf"))
    ...
    plot_tracer_against_reference(config.xcr, config.ycr, tracer, tracer_ref, e_2, e_inf, Path(figure_dir, "blossey_ref.pdf"), cfl_max, config.dx)
...
if __name__ == "__main__":
    ...
    plot_blossey(config.xcr, config.ycr, vx, vy, tracer, Path(run_figures_path, "blossey_0.pdf"))
