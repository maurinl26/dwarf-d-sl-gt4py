...
def init_uniform(U: float, V: float, nx: int, ny: int):
    ...
    Args:
        U (float): Prescribed initial velocity on x
        V (float): Prescibed initial velocity on y
        nx (int): number of cells on x 
        ny (int): number of cells on y

    Returns:
        Tuple[np.ndarray]: initalized velocities
    """
    # Vitesses
...
def one_step_driver(
    ...
    """Driver to perform one step with prescribed velocity
    ...
    Args:
        ...
        interp_function (callable, optional): _description_. Defaults to interpolate_lin_2d.
    ...
    # logging.info("Courant diagnostics")
