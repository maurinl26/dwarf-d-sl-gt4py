...
def init_uniform(U: float, V: float, nx: int, ny: int):
    """Initialization with a uniform velocity

    Args:
        U (float): prescribed velocity on x
        V (float): prescribed velocity on y
        nx (int): number of cells on x
        ny (int): number of cells on y
...
def sl_driver(
    ...
    while t < model_endtime:
        ...
        logging.debug(f"Computing Courant number with dx {config.dx} and dt {config.dt}")
        ...
        if t >= (T / 4) and t < (T / 4) + config.dth:
            plot_blossey(config.xcr, config.ycr, vx, vy, tracer, f"./figures/uniform_advection/uniform_advection_{t:.02f}.pdf")
